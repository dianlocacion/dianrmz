...
/*!
reestructurado por  giovanni rodriguez
  Dian

@ ing.giovanni.rodriguez@gmail.com
...
function showResult(result) {
	...
	info = info.join('\n');
  var Dian_el_json_result=escape(JSON.stringify(result.parsed.fields, false, 4));
  var Dian_elcodigo_mrz=info;
	if (result.error) {
	  html = ['<div class="error">', escape(result.error), '</div>', '<pre>', escape(info), '</pre>'];
	} else {
	  if (result.parsed.valid) {
		html = ['<pre>',Dian_el_json_result, '</pre>', '<pre>', escape(info), '</pre>'];
	  } else {
	   ...
	   document.getElementById("DianModal").innerHTML='No se pudo analizar el texto ocrizado:'+ 'b<br>'+" "+escape(info);
	  
	  }
	}
	$('#parsed').html(html.join('\n'));
	document.getElementById("DianModal").innerHTML=Dian_el_json_result;
	showImages(['painted'] );
   
	console.log(Dian_el_json_result);
	console.log(Dian_elcodigo_mrz);
